...
export class UsuarioRepository {
    ...
    private async _guardarUsuario(usuario: Omit<Usuario, "idUsuario">): Promise<Usuario> {
        const result = await conexion.query(
            'insert into "usuario" ("nombreUsuario", "correo", "contrasena", "estado", "fkIdRol") values ($1, $2, $3, $4, $5) returning "idUsuario"',
            [usuario.nombreUsuario, usuario.correo, usuario.contrasena, usuario.estado, usuario.fkIdRol]
        );
        if (!result.rows[0]) throw new Error("No se pudo obtener el id del usuario insertado");
        return { ...usuario, idUsuario: result.rows[0].idUsuario };
    }
    ...
    guardarUsuario = withTryCatchThrow(this._guardarUsuario.bind(this), "Error al guardar el usuario.");
    actualizarUsuario = withTryCatchThrow(this._actualizarUsuario.bind(this), "Error al actualizar el usuario.");
    eliminarUsuario = withTryCatchThrow(this._eliminarUsuario.bind(this), "Error al eliminar el usuario.");
}