...
export class PedidoRepository {
    ...
    private async _obtenerPedidosPorMesa(idMesa: number): Promise<Pedido[]> {
        ...
        private async _guardarPedido(pedido: Omit<Pedido, "idPedido">): Promise<Pedido> {
            const result = await conexion.query(
                'insert into "pedido" ("fechaPedido", "horaPedido", "estadoPedido", "total", "fkIdCliente", "fkIdMesa", "fkIdEmpleado") values ($1, $2, $3, $4, $5, $6, $7) returning "idPedido"',
                [pedido.fechaPedido, pedido.horaPedido, pedido.estadoPedido, pedido.total, pedido.fkIdCliente, pedido.fkIdMesa, pedido.fkIdEmpleado]
            );
            return { ...pedido, idPedido: result.rows[0].idPedido };
        }
    ...
    guardarPedido = withTryCatchThrow(this._guardarPedido.bind(this), "Error al guardar el pedido.");
    actualizarPedido = withTryCatchThrow(this._actualizarPedido.bind(this), "Error al actualizar el pedido.");
    eliminarPedido = withTryCatchThrow(this._eliminarPedido.bind(this), "Error al eliminar el pedido.");
}