...
export const movimientoInventarioSchema = z.object({
    tipoMovimiento: z.nativeEnum(tipoMovimiento).default(tipoMovimiento.ENTRADA),
    cantidad: z.coerce.number().positive("La cantidad debe ser mayor a 0").max(999999.99, "Cantidad excede el límite"),
    ...
    fkIdIngrediente: z.coerce.number().int().positive("ID de ingrediente inválido"),
});

export const movimientoInventarioUpdateSchema = movimientoInventarioSchema.partial().extend({
    idMovimiento: z.coerce.number().int().positive("El ID debe ser positivo"),
});